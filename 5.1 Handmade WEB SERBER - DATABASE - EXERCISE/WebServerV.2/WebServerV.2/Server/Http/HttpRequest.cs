namespace WebServerV._2.Server.Http
{
    ...
    class HttpRequest : IHttpRequest
    {
        ...
        private void ParseRequest(string stringRequest)
        {
            ...
            this.ParseRequestQuery(requestTrimmed);
			this.ParseCookies();
            this.SetSession();
        ...
        private void ParseRequestQuery(string[] requestLines)
        {
            ...
            {
                var requestTrimmed = stringRequest.Trim().Split(Environment.NewLine);
                var emptySpaceIndex = Array.IndexOf(requestTrimmed, string.Empty) + 1;
                string queryTail = string.Empty;

                for (int i = emptySpaceIndex; i < requestTrimmed.Length; i++)
                {
                    queryTail += requestTrimmed[i];
                }

                

                this.ParseQuery(queryTail, this.FormData);
            }
        }
